...
const EditarUsuario = () => {
  ...
  // Cargamos los datos del usuario para rellenar en el formulario
  useEffect(() => {
    const obtenerUsuario = async () => {
      try {
        const res = await instanciaAxios.get(`/inicio/user/${id}`); 
        setUsuario(res.data);
        console.log(usuario);
      } catch (error) {
  ...
  const editarUsuario = async (e) => {
    ...
    try {
      await instanciaAxios.put(`/inicio/edit-user/${id}`, usuario);
    ...
    <div>
      ...
      <div className="container mt-4">
        ...
        <form onSubmit={editarUsuario} className="card shadow p-4">
